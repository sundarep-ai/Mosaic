...
| **Settings** | Profile picture upload, Income Tracking toggle (Personal and Blended modes only), app mode selector (Personal / Shared / Blended), date format preference, Stay Signed In toggle, password change, and account deletion. |
...
The chosen format applies everywhere dates appear (Home, History, Analytics, Calendar, Insights) and everywhere dates are entered (Add Expense, Add Income, Analytics custom date range). Date inputs use a custom text field with auto-slash insertion and format-aware placeholder, ensuring consistent formatting regardless of OS locale. A calendar icon button next to the field opens the native OS date picker for quick selection — choosing a date from the picker syncs it back to the formatted text field. Database storage always remains ISO `YYYY-MM-DD`.
...
backend/
  ...
  migrate_expenses.py      # Bulk import expenses from .xlsx or .csv
  migrate_income.py        # Bulk import income from .xlsx or .csv (Personal/Blended only)
